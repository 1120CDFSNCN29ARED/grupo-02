...
const postsController = {
  create: async (req, res) => {
    ...
    if (
      req.params.productType === "vehicle" ||
      req.params.productType === "part"
    ) {
  ...
  delete: async (req, res) => {
    ...
    await postsService.update(req.params.postID, {
      active: false,
      published: false,
    });
    const product = await productsService.update(post.productID, {
      active: false,
    });
    let vehicle = null;
    let part = null;
    if (product.productType === "part") {
      ...
      vehicle = await vehiclesService.update(product.vehicleID, {
        active: false,
      });
    }
    ...
    if (req.query) {
      ...
      if (req.query.productType) {
        matchedPosts = matchedPosts.filter(
          (post) => post.post.productType === req.query.productType
        );
        if (req.query.productType === "vehicle") {
          if (req.query.inputKMFrom) {
            matchedPosts = matchedPosts.filter(
              (post) => post.post.kilometers >= req.query.inputKMFrom
            );
          }
          if (req.query.inputKMTo) {
            matchedPosts = matchedPosts.filter(
              (post) => post.post.kilometers <= req.query.inputKMTo
            );
          }
          if (req.query.inputYearFrom) {
            matchedPosts = matchedPosts.filter(
              (post) => post.post.year >= req.query.inputYearFrom
            );
          }
          if (req.query.inputYearTo) {
            matchedPosts = matchedPosts.filter(
              (post) => post.post.year <= req.query.inputYearTo
            );
          }
          if (req.query.type) {
            matchedPosts = matchedPosts.filter(
              (post) => post.post.type === req.query.type
            );
          }
        }
      }
      ...
      if (req.query.inputPriceFrom) {
        matchedPosts = matchedPosts.filter(
          (post) => post.post.price >= req.query.inputPriceFrom
        );
      }
      if (req.query.inputPriceTo) {
        matchedPosts = matchedPosts.filter(
          (post) => post.post.price <= req.query.inputPriceTo
        );
      }
      if (req.query.brand) {
        matchedPosts = matchedPosts.filter(
          (post) => post.post.brandID == req.query.brand
        );
      }
      if (req.query.model) {
        matchedPosts = matchedPosts.filter(
          (post) => post.post.modelID == req.query.model
        );
      }
      if (req.query.state) {
        matchedPosts = matchedPosts.filter(
          (post) => post.post.state == req.query.state
        );
      }
      let search =
        req.query.vehicleSearchList ||
        req.query.partSearchList ||
