...
const brandsController = {
  ...
  delete: async (req, res) => {
    ...
    let brand = await brandsService.findByPk(req.params.brandID);
    if (brand) {
      const models = await modelsService.findByBrandID(brand.brandID);
      if (models.length > 0) {
        result.meta.status = 403;
        result.error = {
          status: 403,
          message: `This brand has associated models, please delete the models first`,
          data: models,
        };
      } else {
        brand = await brandsService.delete(
          req.params.brandID,
          req.query.confirm
        );
        result.meta.status = 200;
