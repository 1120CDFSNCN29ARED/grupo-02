...
async function populateDB(db) {
	...
	const part = await db.Part.create({modelID: partModel.modelID, partSerialNumber: "abc123", car: true});
	...
	const partPost = await db.Post.create({title: "Test Part Title", description: "Test Description", published: true,publishedDate: new Date(), price: 1234,onSale: true,
		discount: 10, stock: 10, rating: 3, state: "Nuevo", featured: true, sellerID: user.userID, locationID: 1, productID: partProduct.productID});
