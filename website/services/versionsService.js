...
const versionsService = {
  findAll: async (conditions) => {
    return await db.VehicleVersion.findAll({
      where: conditions,
      order: [["version_name", "ASC"]],
    }).catch((error) => error);
  },
  findByPk: async (id) => {
  ...
  findByBrandID: async (id) => {
    return await versionsService.findAll({ brandID: id, active: true });
  },
  findByModelID: async (id) => {
    return await versionsService
      .findAll({ modelID: id, active: true })
      .catch((error) => error);
  },
  create: async (data) => {
  ...
  delete: async (id, confirm) => {
    ...
    const products = await version
      .getProducts({ where: { active: true } })
      .catch((error) => error);
