...
const vehiclePostCreationValidator = () => {
  const postCreationValidator = [
    ...
    body("brandID")
      ...
      .bail()
      .custom(async (value) => {
        const brand = await brandsService
          .findByPk(value)
          .catch((error) => error);
        if (!brand) {
    ...
    body("stock")
      .notEmpty()
      .withMessage("Debe ingresar la cantidad de stock")
      .isInt({ gt: 0 })
      .withMessage("Debe ingresar un número mayor a cero"),
    body("rating")
      .notEmpty()
      .withMessage("Debe ingresar un rating")
      .bail()
      .isNumeric()
      .isIn([0, 1, 2, 3, 4, 5])
      .withMessage("Debe ingresar un número (entre 0 y 5)"),
    body("price")
      .notEmpty()
    ...
    body("discount")
      .notEmpty()
      .withMessage(
        "Debe ingresar el descuento. Si no desea aplicar un descuento, ingrese 0"
      )
      .bail()
      .isNumeric()
      .withMessage("Debe ingresar un número")
      .bail()
      .isFloat({ gt: -1, lt: 100 })
      .withMessage("Debe ingresar un número entre 0 y 100"),
    body("provinceID")
      .notEmpty()
      .withMessage("Debe ingresar la provincia")
      .bail(),
    body("locationID")
      .notEmpty()
      .withMessage("Debe ingresar la ciudad")
      .bail()
      .custom(async (value) => {
    ...
    body("versionID")
      ...
      .bail()
      .custom(async (value) => {
        const version = await versionsService
          .findByPk(value)
          .catch((error) => error);
        if (!version) {
    ...
    body("gearType")
      ...
      .bail()
      .isIn(["automática", "manual"])
      .withMessage("Debe ingresar automática o manual"),
    body("color").notEmpty().withMessage("Debe seleccionar un color"),
    body("kilometers")
      .notEmpty()
      .withMessage("Debe ingresar los kilometros")
      .bail()
      .isNumeric()
      .withMessage("Debe ingresar un número")
      .bail()
...
const partPostCreationValidation = async (req, res, next) => {
  ...
  if (errors.isEmpty()) {
    ...
    if (req.url.includes("create")) {
      const brands = await brandsService.findByProductType({
        makesParts: true,
      });
      return res.render("createPost", {
        brands,
...
module.exports = {
  vehiclePostCreationValidator,
  vehiclePostCreationValidation,
  partPostCreationValidator,
  partPostCreationValidation,
};
